...
public class ATV_Editor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        writeToPNG = EditorGUILayout.Toggle("Export PNG files", writeToPNG);
        GUILayout.Space(2);
    ...
    IEnumerator ExportFrames()
    {
        ...
        if ((MeshToBake != null) && (MeshToBake.childCount != 0))
        {
            ...
            if (exportVAT)
            {
                    ...
                    if (VariableTopology == TopologyType.Variable)
                    {
                        ...
                        if (localMeshFilter != null)
                        {
                            ...
                            for (int targetIndex = 0; targetIndex < maxTriangleCount * 3; targetIndex++)
                            {
                                ...
                                Color newVertexColor = Color.black;
                ...
                if (writeToPNG)
                {
                    File.WriteAllBytes(finalExportPath + ExportFilename + "_position.jpg", positionTexture.EncodeToJPG());
                ...
                if (writeColorTexture)
                {
                    ...
                    if (writeToPNG)
                    {
                        File.WriteAllBytes(finalExportPath + ExportFilename + "_color.png", colorTexture.EncodeToPNG());
                    }
                }
            ...
            Texture2D resColorTexture = Resources.Load<Texture2D>(ExportFilename + "_color");
            ...
            if (resNormalTexture == null)
                Debug.Log("Can't load normal texture " + finalExportPath + ExportFilename + "_normal.asset");
            if (resColorTexture == null)
                Debug.Log("Can't load color texture " + finalExportPath + ExportFilename + "_color.asset");

            newMaterial.SetTexture("_VAT_positions", resPosTexture);
            newMaterial.SetTexture("_VAT_normals", resNormalTexture);
            newMaterial.SetTexture("_VAT_colors", resColorTexture);
